...
doWomenOrMenTendToPerformBetter <- studentCourseAssessmentInfoTables %>% 
  group_by(gender) %>% 
  summarize(median = median(student_score),
            mean = mean(student_score))

view(doWomenOrMenTendToPerformBetter)
# --
...
doDisabledOrNonDisabledPeopleTendToPerformBetter <- studentCourseAssessmentInfoTables %>% 
  group_by(student_has_a_disability) %>% 
  summarize(median = median(student_score),
            mean = mean(student_score))

view(doDisabledOrNonDisabledPeopleTendToPerformBetter)
# --
...
doOlderOrYoungerPeoplePerformBetter <- studentCourseAssessmentInfoTables %>% 
  group_by(student_age) %>% 
  summarize(median = median(student_score),
            mean = mean(student_score))

view(doOlderOrYoungerPeoplePerformBetter)
# --
...
doLessImpoverishedStudentsPerformBetterOrWorse <- studentCourseAssessmentInfoTables %>% 
  group_by(index_of_multiple_deprivation_for_a_uk_region) %>% 
  summarize(median = median(student_score),
            mean = mean(student_score))

view(doLessImpoverishedStudentsPerformBetterOrWorse)
# --
...
doMoreEducationStudentsPerformBetter <- studentCourseAssessmentInfoTables %>% 
  group_by(student_highest_education_level_on_entry_to_the_module) %>% 
  summarize(median = median(student_score),
            mean = mean(student_score))

view(doMoreEducationStudentsPerformBetter)
# --



# -- Violin plot for if educational attainment affects student scores
ggplot(studentCourseAssessmentInfoTables, aes(x = assessment_id, y = student_score, fill = student_highest_education_level_on_entry_to_the_module)) + facet_grid(~ student_highest_education_level_on_entry_to_the_module) + geom_violin() +labs(fill="Highest education level on entry to the module") +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
...
abline(3.2827189, 0.0004165, lwd = 2, col = "red")
