...
studentAssessmentTable$student_score <- replace(studentAssessmentTable$student_score, is.na(studentAssessmentTable$student_score), 0)

# Change 0/1 values in the student assessment table to No/Yes for if result is transferred from a previous presentation
studentAssessmentTable$assessment_result_same_as_previous_attempt <- ifelse(studentAssessmentTable$assessment_result_same_as_previous_attempt == 1, 'Yes', 'No')
...
# Correct wrongly entered student ages for 55<=
studentInfoTable[[8]] <- replace(studentInfoTable$student_age, studentInfoTable$student_age == '55<=', ">55")

# Handle wrongly entered IMD band value (missing % sign for 10-20)
studentInfoTable[[7]] <- replace(studentInfoTable$index_of_multiple_deprivation_for_a_uk_region, studentInfoTable$index_of_multiple_deprivation_for_a_uk_region == '10-20', "10-20%")

# Handle missing IMD band values
# Get a list of what unique values we have for the IMD band values
filterListForRegion <- unique(studentInfoTable$index_of_multiple_deprivation_for_a_uk_region)
filterListForRegion
...
whatAreTheNamesOfTheseRegions <- unique(whichRegionsHaveAMissingValue$region_student_lived_in_while_taking_the_module)
whatAreTheNamesOfTheseRegions

# Get the number of regions that have missing values for the IMD value
whatIsTheNumberOfTheseRegions = length(whatAreTheNamesOfTheseRegions)
...
names(mostCommonOnes) <- whatAreTheNamesOfTheseRegions
mostCommonOnes
...
mergedAssessmentTableAndStudentAssessmentTableAndStudentRegistrationTable <- merge(x = mergedAssessmentTableAndStudentAssessmentTable, y = studentRegistrationTable, by.x=c("student_id", "module_id", "presentation_id"), by.y=c("student_id", "module_id", "presentation_id"))
...
mergedAssessmentTableAndStudentAssessmentTableAndStudentRegistrationTableAndStudentInfoTable <- merge(x = mergedAssessmentTableAndStudentAssessmentTableAndStudentRegistrationTable, y = studentInfoTable, by.x=c("student_id", "module_id", "presentation_id"), by.y=c("student_id", "module_id", "presentation_id"))
...
mergedAssessmentTableAndStudentAssessmentTableAndStudentRegistrationTableAndStudentInfoAndCoursesTable <- merge(x = mergedAssessmentTableAndStudentAssessmentTableAndStudentRegistrationTableAndStudentInfoTable, y = coursesTable, by.x=c("module_id", "presentation_id"), by.y=c("module_id", "presentation_id"))
...
mergedAssessmentTableAndStudentAssessmentTableAndStudentRegistrationTableAndStudentInfoAndCoursesTable$assessment_days_since_the_module_began_due_date <- ifelse(is.na(mergedAssessmentTableAndStudentAssessmentTableAndStudentRegistrationTableAndStudentInfoAndCoursesTable$assessment_days_since_the_module_began_due_date), mergedAssessmentTableAndStudentAssessmentTableAndStudentRegistrationTableAndStudentInfoAndCoursesTable$module_days_it_goes_for, mergedAssessmentTableAndStudentAssessmentTableAndStudentRegistrationTableAndStudentInfoAndCoursesTable$assessment_days_since_the_module_began_due_date)
# --
